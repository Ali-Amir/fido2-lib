...
async function validateCounter() {
  ...
  console.warn("Validating counter! prevCounter=", prevCounter, " counter=", counter);
  console.log("Validating counter! prevCounter=", prevCounter, " counter=", counter);

	if (counter <= prevCounter && counterSupported) {
		throw new Error("counter rollback detected. got: ", prevCounter, " expected: ", counter);
	}
...
async function validateAudit() {
	...
	for (let kv of authnrData) {
      ...
      console.error("Oh noo!!!");
			throw new Error(`This is new version. internal audit failed: ${val} was not validated. kv=${kv}. journal=${journal}.`);
		}
	}
